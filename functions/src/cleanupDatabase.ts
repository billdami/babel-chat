...
const funcCleanupDatabase = (env: 'production' | 'development'): HttpsFunction =>
  functions.https.onRequest(async () => {
    ...
    // TODO does this work w/o get()'ing?
    const staleUsers = db.ref('users').orderByChild('dateLastActive').endBefore(cutoffTime);
    // TODO does this work for a "list" of users?
    await staleUsers.ref.remove();
