...
const funcCleanupDatabase = (env: 'production' | 'development') =>
  functions.https.onRequest(
    async (req: functions.https.Request, res: functions.Response<string>) => {
      ...
      const staleUsers = db.ref('users').orderByChild('dateLastActive').endBefore(cutoffTime); // TODO does this work w/o get()'ing?
      await staleUsers.ref.remove(); // TODO does this work for a "list" of users?
      ...
      // delete any /chat_messages whose split key (on "_") of 2 user keys does not a matching /users key for BOTH
      const chatMessages = await db.ref('chat_messsages').get();
          ...
          chatMessageDeletes.push(
            new Promise(async (resolve) => {
              try {
                ...
                if (keys?.length === 2) {
                  const user1 = await db.ref(`users/${keys[0]}`).get();
                  const user2 = await db.ref(`users/${keys[1]}`).get();
                  if (!user1.exists() && !user2.exists()) {
                    await chatMsgs.ref.remove();
                    return resolve();
                  }
                }
              } catch (err) {
      ...
      // TODO [future] delete old/orphaned firebase anonymous sessions that no longer have an associated user
